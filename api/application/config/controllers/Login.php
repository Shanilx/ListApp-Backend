...
class Login extends REST_Controller {
  ...
  public function SignUp_post()
  {
    ...
    if($mobile_number =='' || $role == '' || $password == ''  || $shop_name =='' ||  $device_type =='' || $device_token ==''  || $mobile_unique_id==''){
...
{ 
  ...
  else
  {
    ...
    {  
      ...
      {
          $post_arr=array();
          $post_arr[]=array('notification_id'=>$msg['notification_id'],'type'=>$msg['notification_type'],'title'=>$msg['notification_title'],'meassage'=>$msg['notification_message'],'read'=>$msg['is_read'],'date'=>date('d-m-Y', strtotime($msg['notification_date'])),'time'=> date('h:i A', strtotime($msg['notification_date'])),'current_date'=>date('d-m-Y'));
          $this->users_model->InsertData('notifications',$post_arr);
      } 
      else
      {
        $error = array('error'=>'1','message'=>'Your session has been expired');
