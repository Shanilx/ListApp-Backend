...
class Login extends CI_Controller {
	...
	{ 
     ...
     { 
...

     	{
     		...
     		// echo $b.'   =  '. decode('S7Mys1IyNDI2MTVTsgYA');
...

     		{
     			...
     			//$hashedPassword = $this->encryption->encode($userPassword);

     			//$hashedPassword = encode($userPassword);
     			...
     			$where = array(
     				...
     				'pwd_without_encode'=>trim($hashedPassword)
     ...
     {  
		...
		//$hashedPassword = encode($userPassword);     //encode function in commen_helper 

		$hashedPassword = md5($userPassword.''.$salt);     //encode function in commen_helper 
	...
	private function check_admin_login(){
	    ...
	    $r=$this->session->userdata('admin_id');
...

	{
		...
		$where_r=array('status'=>'Active','role'=>3);

		$where_s=array('status'=>'Active','role'=>2);

		$where_o=array('status'=>'Active','role'=>4);

		$where_c=array('status'=>'Active','role'=>5);
