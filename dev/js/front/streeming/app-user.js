...
//////////////////////////////////////////////////////////
        ...
        function notifyIfUserLeaveCall(session, userId, reason, title) {
          ...
          window.location.href= base_url + 'patient/profile#RateReview#'+ app_id;
          //setTimeout(function(){ window.location.href= base_url + '/patient/profile#RateReview#'+ app_id; }, 4000);

          /*----redirect---*/
          var loc = window.location;
          var pathArray = window.location.pathname.split( '/' );
          qbApp.MsgBoard.update('login_tpl', {name: app.callee.full_name});
          window.location.href= loc.protocol + '//' + loc.host +'/'+ pathArray[1] + '/patient/profile';
  ...
  // });
     ...
     $( document ).ready(function() {
                ...
                if(_.isEmpty(app.caller))
                {
                      ...
                      //  alert( QBApp.appId);
                        ...
                        QB.chat.connect({
                           ...
                           // alert('login to quickblox');
                            if(err !== null)
                             {
                               // alert('if user selected');
                                app.caller = {};
                    ...
                    {   //  alert('hhhh');
                         ...
                         // user.name = 'User 2';
                          ...
                          if ($el.hasClass(classNameCheckedUser)) {  //alert(app.callees);
                              delete app.callees[user.id];
                              $el.removeClass(classNameCheckedUser);
                          } else {
                              app.callees[user.id] = user.name;
       ...
       $(document).on('click', '.j-accept', function() {
          ...
          $("#start_time").val(time);
          // QBUsers=JSON.parse(document.getElementById('qbuser_text').innerHTML);
          QBUsers=JSON.parse(document.getElementById('qbuser_text').value);
          //alert(QBUsers);
            ...
            app.currentSession.getUserMedia(mediaParams, function(err, stream) {
                if (err || !stream.getAudioTracks().length || !stream.getVideoTracks().length) {
                  ...
                  app.currentSession.stop({});
                    ...
                    /*-----place trainer name-----*/
                    var userInfo = _.findWhere(QBUsers, {id: app.currentSession.initiatorID});
                    //document.getElementById('qb_caller').innerHTML='Trainer -'+userInfo.full_name
        ...
        QB.webrtc.onCallListener = function onCallListener(session, extension) { //window.location.href="Streaming/54";

         /*-------get session user-----*/
           $.ajax({
           type: "POST",
           //url: base_fitstreem+"website/classdetail/Getsessionuser",
           url: base_url+"user/Getsessionuser",
           //data:'session_id='+session.ID,
           data:'app_id='+ $('#app_id').val(),
           success: function (data) { //alert(data);
           //QBUsers=data;
           document.getElementById('qbuser_text').innerHTML =data;
           $('#qbuser_text').val(data);
           }
          });
         //document.getElementById('qbuser_text').value=[{id:"16275482",login:"t@gmail.com",password:"Fit123Quick",full_name:"tag craft",colour:'FD8209'},{id:"16354248",login:"beta@gmail.com",password:"Fit123Quick",full_name:"beta",colour:'FD8209'}];
