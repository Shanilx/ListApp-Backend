    ...
    /** when DOM is ready */
        ...
        var ui = {
              ...
              updTimer: function() {
                  ...
                  if ((this.callTime/1000) % 60 == 1)
                  { 
                     QB.chat.send('22395601','hhhhhh');

                  //alert('me');
                     // $('#calorie_timer').removeClass('hidden').text((this.callTime/1000));
                      /*calculate calorie*/
                      // var video_user=document.getElementsByClassName('j-callees__callee_video');
                      // for(var k=0; k<video_user.length;k++)
                      // {
                      //    var timer_calorie=this.callTime/1000;
                      //    var split_id=(video_user[k].id).split("_");
                      //    var headerweight=document.getElementById('weight-'+split_id[2]).value;
                      //    var headermet=document.getElementById('met-'+split_id[2]).value;
                      //    var calusername=document.getElementById('calusername-'+split_id[2]).value;
                      //    var calorie_count=timer_calorie*(headermet*3.5*headerweight)/200;
                      //    document.getElementById('headerdata-'+split_id[2]).innerHTML=calusername+' Calorie:'+Math.round(calorie_count);
                      // }
                  }
                 //$('#calorie_timer').removeClass('hidden').text((this.callTime/1000)*(12*4*64)/200);
        ...
        function showErrorAccessPermission(error) { //alert('cameraal');
          var errorTitle = 'Error: ',
            errorMsg = 'Failed to get access to your camera or microphone. Please check your hardware configuration.';
            //alert(errorMsg);

          if(error && error.message) {
            if( error.message == 'The object can not be found here.')
            {
                alert(errorTitle + 'Please check your camera and microphone connection.');
            }
            else
            {
...
//////////////////////////////////////////////////////////
        ...
        function notifyIfUserLeaveCall(session, userId, reason, title) { 
              ...
              var sent_note = $("#sent_note").val();
              sent_note = $(sent_note).text(); // html to text   
              sent_note = sent_note.replace(/\r?\n|\r/gm," "); // remove line breaks   
              sent_note = sent_note.replace(/\s\s+/g, " ").trim(); // remove double spaces


              var TextGrab = CKEDITOR.instances['treatment_note'].getData();
              ...
              {
                $("#is_cancelled").val('1');
                alert('Please send the treatment note to patient');
              }
              else if(sent_note!=TextGrab)
              {
                $("#is_cancelled").val('1');
                alert('Please send the updated treatment note to patient');
              }
              else
              { 
                   /*----redirect---*/
                  ...
                  var pathArray = window.location.pathname.split( '/' );
                  qbApp.MsgBoard.update('login_tpl', {name: app.caller.full_name});*/
                  window.location.href= loc.protocol + '//' + loc.host +'/'+ pathArray[1] + '/doctor/profile';
      ...
      //       alert('click on user then this fucntion is called');
      //           var $el = $(this),
      //           usersWithoutCaller = [],
      //           user = {},
      ...
      //                    */
      //                   app.caller = {
      //                       id: '<?php echo $this->session->userdata("quickblox_id")?>',
      //                       login: '<?php echo $this->session->userdata("login")?>',
      //                       password: '<?php echo $this->session->userdata("password")?>',
      ...
      //   });
        ...
        $(document).on('click', '.j-logout', function() {
            QB.chat.disconnect();
            //window.location.href= loc.protocol + '//' + loc.host +'/'+ pathArray[1] + '/front/home';
          ...
          var videoElems = '',
           ...
           /*---------------login to qb-------------------*/
                ...
                classNameCheckedUser = 'users__user-active';
                      ...
                      var qb_login=document.getElementById('qb_login').value;
                     // alert(qb_login);
                      var qb_password=document.getElementById('qb_password').value;
         ...
         //  console.log( app.callees);
          ...
          } else 
          {
            ...
            if ( _.isEmpty(app.callees) ) {
              ...
              //alert( app.currentSession.ID );
              app.currentSession.getUserMedia(mediaParams, function(err, stream) { 
                if (err || !stream.getAudioTracks().length || !stream.getVideoTracks().length) { //alert('if');
                  var errorMsg = '';

                  if(err && err.message) {
                      if( err.message == 'The object can not be found here.')
                      {
                          errorMsg += 'Error: ' + 'Please check your camera and microphone connection.';
                      }
                      else
                      {
                  ...
                  app.currentSession.call({}, function(error) {
                    if(error) {
                      ...
                      $.ajax({
                      type: "POST",
                      //url: base_fitstreem+"website/classdetail/updatesession",
                      url: base_url+"user/updatesession",
                      //data:'session_id='+app.currentSession.ID+'&class_id='+pathArray[3],
                      data:'session_id='+app.currentSession.ID+'&app_id='+app_id,
       ...
       //
        ...
        /**------------------------------- Hangup---------------------------------------- */
        $(document).on('click', '.j-hangup', function() { // alert('hang up');
        ...
        $(document).on('click', '.j-accept', function() {
          ...
          $("#start_time").val(time);
            ...
            var app_id = $("#encoded_app_id").val();
            $.ajax({
                   type: "POST", 
                   url: update_start_time,
                   data: {start_time:start_time, app_id: app_id},
                   success: function(data) {
        ...
        /** ------------------------Mute / Unmute cam / mic ----------------------------*/
        $(document).on('click', '.j-caller__ctrl', function() { //alert('button');
            var $btn = $(this),
        ...
        //  $(document).on('click', '.startclasscalorie', function() { alert('calorie');

        // });

        /** ---------------------set main video ----------------------------------*/
        $(document).on('click', '.j-callees__callee_video', function() { //alert('swicth');
            var $that = $(this),
                userID = +($(this).data('user')),
                classesName = [],
                activeClass = [];

            if( app.currentSession.peerConnections[userID].stream && !_.isEmpty( $that.attr('src')) ) { //alert(userID);
                if( $that.hasClass('active') ) {
                    ...
                    //var headername=document.getElementById('headerdata-'+userID).innerHTML;
                    //document.getElementById('headeruser').innerHTML=headername;
        ...
        QB.webrtc.onCallStatsReport = function onCallStatsReport(session, userId, stats) { //alert('stats');
        ...
        QB.webrtc.onRejectCallListener = function onRejectCallListener(session, userId, extension) {
          ...
          /** It's for p2p call */
          if(session.opponentsIDs.length === 1) {
              alert('Patient has declined the call');
              setTimeout(function(){ window.location.href= base_url + 'doctor/profile'; }, 4000);
        ...
        QB.webrtc.onSessionConnectionStateChangedListener = function onSessionConnectionStateChangedListener(session, userID, connectionState) { 
          ...
          if(connectionState === QB.webrtc.SessionConnectionState.CLOSED){
              ...
              if( _.isEmpty(app.currentSession) || isCallEnded ) {
                  if(callTimer) { 
                      var timer_val = $('#timer').text();
                      var app_id = $("#encoded_app_id").val();
                      $.ajax({
                         type: "POST", 
                         url: update_timer_val,
                         data: {timer_val:timer_val, app_id: app_id},
                         success: function(data) {
